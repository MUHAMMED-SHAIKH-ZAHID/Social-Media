...
import { addMessageAPI, getMessagesAPI, userChatsAPI, userCreateChatsAPI } from "../../url";
...
export const createChats = createAsyncThunk('createChats', async (body) => {
    const token = localStorage.getItem('token')
    return await axios.post(`${userCreateChatsAPI}`,body, { headers: { 'authorization': 'Bearer ' + token } }).then(({ data }) => {
        return data
    }).catch((err) => {
        console.log(err)
    })
